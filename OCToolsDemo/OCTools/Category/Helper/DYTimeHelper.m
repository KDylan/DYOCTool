...
#import "DYTimeHelper.h"

static DYTimeHelper *instance = nil;

@implementation DYTimeHelper
...
+ (DYTimeHelper *)getInstance
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        instance = [[DYTimeHelper alloc] init];
...
-(NSString *)dealWithStayTimeStartTime:(NSDate *)startTime endTime:(NSDate *)endTime{
    ...
    if(cmps.hour>=1){
        ...
        //  一分钟以内
        return[NSString stringWithFormat:@"stay:%zds",cmps.second];
    }
}
/**比较时间*/
...
+(NSString *)dealTimeDateToString:(NSDate *)dateTime{
    ...
    NSString *currentDateStr = [NSString stringWithFormat:@"%@",[dateFormatter stringFromDate:dateTime]];
...
+(NSString *)getCurrentDateformat:(NSString *)format{
    ...
    return  [self dealTimeDateToString:currentDate format:format];
